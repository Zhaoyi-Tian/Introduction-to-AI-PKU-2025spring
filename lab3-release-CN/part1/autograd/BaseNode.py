...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        def softmax(X,dim):
            ...
            expX = np.exp(X)
            ret = expX / expX.sum(axis=dim, keepdims=True)
        ...
        softmax_score = softmax(scores,-1)
...
class ResLinear(Node):
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        X = self.cache[-1]
        ret1 = X.transpose(0, 2, 1)@grad
        ret1 = np.sum(ret1, axis=0)
        ret2 = np.sum(grad, axis=0)
        self.grad.append(ret1)
        self.grad.append(ret2)
        return grad@self.params[0].T+ grad  # W 的转置
        # 反向传播的时候，与 Linear 相同
