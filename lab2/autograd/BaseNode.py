...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        '''
        # TODO: YOUR CODE HERE
        ret1=self.cache[-1].T@grad
        ret2=np.sum(grad,axis=0)
        self.grad.append(ret1)
        self.grad.append(ret2)
...
class BatchNorm(Node):
    ...
    def __init__(self, indim, momentum: float = 0.9):
        super().__init__("batchnorm", np.ones((indim)), zeros(indim))
...
class LogSoftmax(Node):
    ...
    def backcal(self, grad):
        ...
        softmaxX=np.exp(logsoftmaxX)
        ret=grad - np.multiply(grad.sum(axis=self.dim, keepdims=True), softmaxX)
        return ret
...
class CrossEntropyLoss(Node):
    ...
    def backcal(self, grad):
        ...
        X, y = self.cache[-1], self.y
        ret = np.zeros_like(X)
        indices = np.expand_dims(y, axis=-1)
        x_selected = np.take_along_axis(X, indices, axis=-1)
        np.put_along_axis(ret, indices, -1 / (x_selected+1e-5), axis=-1)
        return grad * ret
