...
def entropy(Y: np.ndarray):
    ...
    for idx in range(len(ufeat)):
        res-=featcnt[idx]/s*np.log2(featcnt[idx]/s)
    return res
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    for i in ufeat:
        mask=feat==i
        root.children[i]=buildTree(X[mask],Y[mask],unused,depth-1, purity_bound, gainfunc, prefixstr+str(i))
    # 按选择的属性划分样本集，递归构建决策树
