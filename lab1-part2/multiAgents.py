    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in state.getLegalActions(index_of_agent):
                next=state.generateSuccessor(index_of_agent,action)
                v=minimizer(next,depth,index_of_agent+1)[0]
                if v>value:
                    value=v
                    maxiAction=action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in state.getLegalActions(index_of_agent):
                next=state.generateSuccessor(index_of_agent,action)
                v=minimizer(next,depth,index_of_agent+1,alpha, beta)[0]
                if v>value:
                    value=v
                    maxiAction=action
                if v>beta:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            if index_of_agent < n - 1:
                ...
                for action in state.getLegalActions(index_of_agent):
                    next = state.generateSuccessor(index_of_agent, action)
                    v = maximizer(next, depth - 1, 0,alpha, beta)[0]
                    if v < value:
                        value = v
                        miniAction = action
                    if v < alpha:
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        class Node:
            ...
            def __init__(self, data):
                ...
                self.statevalue = data[0]           # 该节点对应的 <class 'GameState' (defined in pacman.py)>
                self.numerator = data[1]            # 该节点的分数
                self.denominator = data[2]          # 该节点的访问次数游戏状态,
        ...
        def Selection(cgs, cgstree):
            ...
            if len(legals) == 0:
                return cgs,cgstree
            map = {"North": "north", "East": "east", "West": "west", "South": "south", "Stop": "stop"}
            children_list = []
            for action in legals:
                action1 = map[action]
                if getattr(cgstree, action1) is None:
                    return cgs, cgstree
                else:
                    children_list.append((getattr(cgstree, action1),action))
            best_child_state,best_action=best_UCT(children_list)
            later=getattr(cgstree, map[best_action])
            return Selection(best_child_state,later)
        def Expansion(cgstree):
            ...
            for action in legal_actions:
                action1=map[action]
                if getattr(cgstree, action1) is None:
                    child=Node((cgstree.statevalue.generateSuccessor(0,action),0,1))
                    child.parent=cgstree
                    setattr(cgstree,action1,child)
